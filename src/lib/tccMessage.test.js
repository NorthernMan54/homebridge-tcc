...
describe('tccMessage helpers', () => {
  ...
  test('SOAP message builders create correct structure', () => {
    ...
    expect(auth.AuthenticateUserLogin.username).toBe('user');

    const loc = tccMessage.GetLocationsMessage('session');
    expect(loc.GetLocations.sessionID).toBe('session');

    const comm = tccMessage.GetCommTaskStateMessage('s', 42);
    expect(comm.GetCommTaskState.commTaskID).toBe(42);

    const getTherm = tccMessage.GetThermostatMessage('s', 7);
    expect(getTherm.GetThermostat.thermostatID).toBe(7);
  });
  ...
  test('ChangeThermostatMessage converts temperatures and respects hold settings', () => {
    ...
    expect(change.systemSwitch).toBe(1);
    expect(change.heatSetpoint).toBe('70'); // Converted to Fahrenheit and rounded
    expect(change.coolSetpoint).toBe(74);
    expect(change.statusHeat).toBe(1);
    ...
    expect(autoMessage.systemSwitch).toBe(4);
    expect(autoMessage.heatSetpoint).toBe(19);
    expect(autoMessage.coolSetpoint).toBe(27);
    expect(autoMessage.statusHeat).toBe(2);
    expect(autoMessage.statusCool).toBe(2);
  });
  ...
  test('systemSwitch, heatSetpoint, and coolSetpoint cover all branches', () => {
    ...
    expect(tccMessage.ChangeThermostatMessage('s', { ThermostatID: 1, TargetHeatingCooling: 0 }, offTherm, false).ChangeThermostatUI.systemSwitch).toBe(2);

    const heatPref = baseThermostat();
    const heatSwitch = tccMessage.ChangeThermostatMessage('s', { ThermostatID: 1, TargetHeatingCooling: 1, TargetTemperature: 22 }, heatPref, false).ChangeThermostatUI.systemSwitch;
    expect(heatSwitch).toBe(1);

    const coolSwitch = tccMessage.ChangeThermostatMessage('s', { ThermostatID: 1, TargetHeatingCooling: 2, TargetTemperature: 18 }, baseThermostat(), false).ChangeThermostatUI.systemSwitch;
    expect(coolSwitch).toBe(3);

    const autoSwitch = tccMessage.ChangeThermostatMessage('s', { ThermostatID: 1, TargetHeatingCooling: 3 }, baseThermostat(), false).ChangeThermostatUI.systemSwitch;
    expect(autoSwitch).toBe(4);

    const undefinedSwitchTherm = baseThermostat();
    undefinedSwitchTherm.device.UI.SystemSwitchPosition = 3;
    const undefinedSwitch = tccMessage.ChangeThermostatMessage('s', { ThermostatID: 1 }, undefinedSwitchTherm, false).ChangeThermostatUI.systemSwitch;
    expect(undefinedSwitch).toBe(3);

    const defaultSwitchTherm = baseThermostat();
    defaultSwitchTherm.device.UI.SystemSwitchPosition = 4;
    const defaultSwitch = tccMessage.ChangeThermostatMessage('s', { ThermostatID: 1, TargetHeatingCooling: 99 }, defaultSwitchTherm, false).ChangeThermostatUI.systemSwitch;
    expect(defaultSwitch).toBe(4);

    const heatTherm = baseThermostat();
    heatTherm.device.UI.SystemSwitchPosition = 1;
    const heatResult = tccMessage.ChangeThermostatMessage('s', { ThermostatID: 1, TargetHeatingCooling: 1, TargetTemperature: 23 }, heatTherm, false).ChangeThermostatUI.heatSetpoint;
    ...
    const heatOff = heatOffMessage.heatSetpoint;
    expect(heatOffMessage.systemSwitch).toBe(2);
    ...
    const heatThreshold = tccMessage.ChangeThermostatMessage('s', { ThermostatID: 1, TargetHeatingCooling: 3, HeatingThresholdTemperature: 17 }, heatThresholdTherm, false).ChangeThermostatUI.heatSetpoint;
    expect(heatThreshold).toBe(17);

    const coolTherm = baseThermostat();
    coolTherm.device.UI.SystemSwitchPosition = 3;
    const coolResult = tccMessage.ChangeThermostatMessage('s', { ThermostatID: 1, TargetHeatingCooling: 2, TargetTemperature: 24 }, coolTherm, false).ChangeThermostatUI.coolSetpoint;
    ...
    const coolOff = coolOffMessage.coolSetpoint;
    expect(coolOffMessage.systemSwitch).toBe(2);
    ...
    const coolThreshold = tccMessage.ChangeThermostatMessage('s', { ThermostatID: 1, TargetHeatingCooling: 3, CoolingThresholdTemperature: 27 }, coolThresholdTherm, false).ChangeThermostatUI.coolSetpoint;
    ...
    const coolThresholdMessage = tccMessage.ChangeThermostatMessage('s', { ThermostatID: 1, TargetHeatingCooling: 2, CoolingThresholdTemperature: 26 }, coolThresholdFallback, false).ChangeThermostatUI.coolSetpoint;
