...
/* - better-queue does not support promises
...
function toHb(thermostat) {
  ...
  response.HeatSetpoint = toCelcius(thermostat.UI.HeatSetpoint, thermostat);
  response.CoolSetpoint = toCelcius(thermostat.UI.CoolSetpoint, thermostat);
...
function toThermostat(value, accessory) {
  return (getThermostat(accessory).UI.DisplayedUnits === "C" ? value : ((value * 9 / 5) + 32).toFixed(1));
}
...
function getThermostat(accessory) {
  // debug("getThermostat", thermostats.LocationInfo.Thermostats);
  if (Array.isArray(thermostats.LocationInfo.Thermostats.ThermostatInfo)) {
    for (const item of thermostats.LocationInfo.Thermostats.ThermostatInfo) {
      ...
      if (item.ThermostatID === accessory.context.ThermostatID) {
        // debug("Found", item);
        return item;
      }
    }
