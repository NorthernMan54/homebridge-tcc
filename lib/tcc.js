...
async function pollDevices(callback) {
  ...
  try {
    ...
    thermostats = await _GetLocationListData.call(this);
    debug("pollDevices - callback", thermostats.hb);
    // debug("pollDevices - callback", devices.LocationInfo.Thermostats.ThermostatInfo);
    callback(null, thermostats);
  } catch (err) {
    console.error("pollDevices Error:", err, thermostats);
...
function getThermostat(accessory) {
  debug("getThermostat - ThermostatID", accessory.context.ThermostatID);
  debug("getThermostat - Thermostat", thermostats.LocationInfo.Thermostats);
...
tcc.prototype.setTargetHeatingCooling = function(accessory, value, callback) {
  ...
  (async () => {
    try {
      ...
      debug("SOAP Message", parser.toXml(soapMessage(soapBody)));
      var {
        response
      } = await soapRequest({
        url: URL,
